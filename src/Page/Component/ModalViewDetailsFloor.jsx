...
import { ButtonGroup, Button, Dialog, DialogHeader, DialogBody, DialogFooter, Input, Typography, Switch, Checkbox } from "@material-tailwind/react";
        ...
        <Dialog open={props.openModalFloorDetails} className={"z-[100]"} handler={() => props.handleOpenModalFloorDetails(handleTurnOffDemo)} size={demo ? "xl" : "sm"}>
            ...
            <DialogBody className="pt-0">
                <div className="flex flex-col xl:flex-row">
                    <div className={demo ? "w-[369px] flex flex-col" : "w-full  flex flex-col"}>
                        <Input variant="standard" label="Tên khu vực đỗ xe" value={nameFloorDetails} readOnly onChange={e => setNameFloorDetails(e.target.value)} />
                        <div className="h-6" />
                        <Input variant="standard" label="Số lượng chỗ đỗ xe" type='number' min={1}  value={totalSlot} readOnly onChange={onChangeTotalSlotsFloorDetails} />
                        <div className="h-6" />
                        <Input variant="standard" label="Số cột" type='number' min={1} readOnly value={rowFloorDetails} onChange={onChangeRowFloorDetails} />
                    ...
                    {demo ?
                        <div className=" overflow-auto">
                            ...
                            <Checkbox id="editfloor-location" label="Chỉnh sửa" color="green" ripple={edit} onClick={() => setEdit(!edit)} />
                            <Button variant="gradient" className="mx-6" color="green" disabled={messAdd} onClick={handleUpdateState}>
                                <span>Cập nhật các vị trí</span>
                            </Button>
                            </div>

                            <div className="h-[450px] flex overflow-auto">
                                {!messAdd ? <ParkingSimulation demo={false} edit={edit} rows={rowFloorDetails} handleChangeState={handleChangeState} listLocation={listParkingLocation} total={totalSlot} /> : <div />}
                            </div>
